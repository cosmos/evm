...
func (k Keeper) TraceTx(c context.Context, req *types.QueryTraceTxRequest) (*types.QueryTraceTxResponse, error) {
	...
	for i, tx := range req.Predecessors {
		...
		// Derived txs are unsigned — fall back to constructing the message from ABCI event fields.
	...
	result, _, err := k.traceTx(ctx, cfg, txConfig, signer, common.BytesToAddress(req.Msg.GetFrom()), tx, req.TraceConfig, false)
	if err != nil {
...
func (k Keeper) TraceBlock(c context.Context, req *types.QueryTraceBlockRequest) (*types.QueryTraceBlockResponse, error) {
	...
	for i, tx := range req.Txs {
		...
		txConfig.TxHash = ethTx.Hash()
		txConfig.TxIndex = uint(i) //nolint:gosec // G115 // won't exceed uint64
		traceResult, logIndex, err := k.traceTx(ctx, cfg, txConfig, signer, common.BytesToAddress(tx.GetFrom()), ethTx, req.TraceConfig, true)
		if err != nil {
...
func unsignedTxAsMessage(fromAddress common.Address, tx *ethtypes.Transaction, baseFee *big.Int) core.Message {
	...
	// If baseFee provided, set gasPrice to effectiveGasPrice: min(tip+base, cap).
	if baseFee != nil {
		effective := new(big.Int).Add(tx.GasTipCap(), baseFee)
		if effective.Cmp(tx.GasFeeCap()) > 0 {
			effective = tx.GasFeeCap()
		}
		gasPrice = effective
	}
	return core.Message{
