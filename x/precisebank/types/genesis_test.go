...
func TestGenesisStateValidate_Basic(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(tt *testing.T) {
			err := tc.genesisState.Validate(testConversionFactor)
...
func TestGenesisStateValidate_Total(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			func() *types.GenesisState {
				fbs := testutil.GenerateEqualFractionalBalances(t, 100, testConversionFactor)
		...
		},
		{
			...
			func() *types.GenesisState {
				fbs, remainder := testutil.GenerateEqualFractionalBalancesWithRemainder(t, 100, testConversionFactor)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(tt *testing.T) {
			err := tc.buildGenesisState().Validate(testConversionFactor)
...
func TestGenesisState_TotalAmountWithRemainder(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			types.FractionalBalances{
				types.NewFractionalBalance(sdk.AccAddress{1}.String(), testConversionFactor.QuoRaw(2)),
				types.NewFractionalBalance(sdk.AccAddress{2}.String(), testConversionFactor.QuoRaw(2)),
			},
			sdkmath.ZeroInt(),
			testConversionFactor,
		},
		{
			...
			types.FractionalBalances{
				types.NewFractionalBalance(sdk.AccAddress{1}.String(), testConversionFactor.QuoRaw(2)),
				types.NewFractionalBalance(sdk.AccAddress{2}.String(), testConversionFactor.QuoRaw(2).SubRaw(1)),
			},
			sdkmath.OneInt(),
			testConversionFactor,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.NoError(t, gs.Validate(testConversionFactor), "genesis state should be valid before testing total amount")
...
func FuzzGenesisStateValidate_NonZeroRemainder(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, count int) {
		...
		fbs, remainder := testutil.GenerateEqualFractionalBalancesWithRemainder(t, count, testConversionFactor)

		t.Logf("count: %v", count)
		t.Logf("remainder: %v", remainder)

		gs := types.NewGenesisState(fbs, remainder)
		require.NoError(t, gs.Validate(testConversionFactor))
	})
}
...
func FuzzGenesisStateValidate_ZeroRemainder(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, count int) {
		...
		fbs := testutil.GenerateEqualFractionalBalances(t, count, testConversionFactor)
