...
func (s *RPCStream) initSubscriptions() {
	...
	if err != nil {
		s.logger.Error("failed to subscribe to block events, RPC streaming degraded", "error", err)
		return
	}

	chLogs, err := s.evtClient.Subscribe(ctx, streamSubscriberName, evmEvents, subscribBufferSize)
	if err != nil {
		s.logger.Error("failed to subscribe to EVM events, RPC streaming degraded", "error", err)
		if unsubErr := s.evtClient.UnsubscribeAll(context.Background(), streamSubscriberName); unsubErr != nil {
			s.logger.Error("failed to unsubscribe", "error", unsubErr)
		}
		return
	}
