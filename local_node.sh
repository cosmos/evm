...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# set custom pruning settings with higher limits
	sed -i.bak 's/pruning = "default"/pruning = "custom"/g' "$APP_TOML"
	sed -i.bak 's/pruning-keep-recent = "0"/pruning-keep-recent = "100000000000000"/g' "$APP_TOML"
	sed -i.bak 's/pruning-interval = "0"/pruning-interval = "100"/g' "$APP_TOML"
