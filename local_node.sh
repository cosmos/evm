...
add_genesis_funds() {
  local keyname="$1"
  evmd genesis add-genesis-account "$keyname" "1000000000000000000000$EXTENDED_DENOM" --keyring-backend "$KEYRING" --home "$CHAINDIR"
}
...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
  ...
  jq '.app_state["staking"]["params"]["bond_denom"]="'$EXTENDED_DENOM'"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  jq '.app_state["gov"]["deposit_params"]["min_deposit"][0]["denom"]="'$EXTENDED_DENOM'"' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  jq '.app_state.erc20.token_pairs=[{contract_owner:1,erc20_address:"0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE",denom:"'$EXTENDED_DENOM'",enabled:true}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
  ...
  evmd genesis add-genesis-account "$VAL_KEY" "100000000000000000000000000$EXTENDED_DENOM" --keyring-backend "$KEYRING" --home "$CHAINDIR"
  ...
  # --------- Finalize genesis ---------
  evmd genesis gentx "$VAL_KEY" "1000000000000000000000$EXTENDED_DENOM" --gas-prices ${BASEFEE}$EXTENDED_DENOM --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$CHAINDIR"
