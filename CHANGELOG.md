...
- [\#1304](https://github.com/cosmos/evm/pull/1304) Use the EIP-2935 window of the preinstalled history storage contract (8191) as the default `history_serve_window`, cap the param at 8192, and re-index the stored block hashes when `MsgUpdateParams` changes the window, so the contract's `get` and `BLOCKHASH` return the same hash. Existing chains keep their window (an unset one still means 8192) until they update the param; see `docs/migrations/v0.7.x_to_v0.8.0.md`.
