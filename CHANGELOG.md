...
- [\#1303](https://github.com/cosmos/evm/pull/1303) Resolve the `earliest` block tag to the earliest available height for state queries (`eth_getBalance`, `eth_getCode`, `eth_getStorageAt`, `eth_getTransactionCount`, `eth_call`, ...) instead of answering from the latest state.
