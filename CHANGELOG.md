...
- [\#1305](https://github.com/cosmos/evm/pull/1305) Make `eth_createAccessList` trace the call: it runs through `TraceCall` with a new native `accessListTracer` and re-estimates the gas for each access list, instead of returning the input list unchanged. State overrides are rejected for now since `TraceCall` can't apply them.
