...
def test_subscribe_basic(evm: Evm):
    """
    test basic subscribe and unsubscribe
    """
    wait_for_port(ports.evmrpc_ws_port(evm.base_port(0)))
    cli = evm.cosmos_cli()
    ...
    async def async_test():
        async with websockets.connect(evm.w3_ws_endpoint()) as ws:
            c = Client(ws)
            ...
            erc20.deploy(w3)
            await transfer_test(c, evm.w3, erc20.contract, erc20.contract.address)
            msg = Contract("TestMessageCall")
            msg.deploy(w3)
            inner = msg.contract.caller.inner()
            begin = time.time()
            await logs_test(c, evm.w3, msg.contract, inner)
