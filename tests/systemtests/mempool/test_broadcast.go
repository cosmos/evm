...
func RunTxBroadcasting(t *testing.T) {
	testCases := []struct {
		name    string
	...
	}{
		{
			...
			actions: []func(TestSuite, *TestContext){
				func(s TestSuite, ctx *TestContext) {
					...
					tx1, err := s.SendTx(t, s.Node(0), signer, 0, s.BaseFeeX2(), nil)
					...
					for _, nodeIdx := range []int{1, 2, 3} {
						func(nodeIdx int) {
							...
							for !found {
								...
								case <-ticker.C:
									pendingTxs, _, err := s.TxPoolContent(nodeID, suite.TxTypeEVM)
									if err != nil {
										// Retry on error
										continue
									}
				...
				func(s TestSuite, ctx *TestContext) {
					...
					tx3, err := s.SendTx(t, s.Node(1), signer, 2, s.BaseFeeX2(), nil)
					...
					for !queuedOnNode1 {
						...
						case <-ticker.C:
							_, queuedTxs, err := s.TxPoolContent(s.Node(1), suite.TxTypeEVM)
							if err != nil {
								continue
							}
					...
					for _, nodeIdx := range []int{0, 2, 3} {
						nodeID := s.Node(nodeIdx)
						pendingTxs, queuedTxs, err := s.TxPoolContent(nodeID, suite.TxTypeEVM)
					...
					tx2, err := s.SendTx(t, s.Node(2), signer, 1, s.BaseFeeX2(), nil)
			...
			actions: []func(TestSuite, *TestContext){
				func(s TestSuite, ctx *TestContext) {
					...
					for !found {
						...
						case <-ticker.C:
							pendingTxs, _, err := s.TxPoolContent(s.Node(0), suite.TxTypeEVM)
							if err != nil {
								continue
							}
							if slices.Contains(pendingTxs, tx1.TxHash) {
					...
					_, err = s.SendTx(t, s.Node(0), signer, 0, s.BaseFeeX2(), nil)
			...
			actions: []func(TestSuite, *TestContext){
				func(s TestSuite, ctx *TestContext) {
					...
					for !found {
						...
						case <-ticker.C:
							pendingTxs, _, err := s.TxPoolContent(s.Node(1), suite.TxTypeEVM)
							if err != nil {
								continue
							}
							if slices.Contains(pendingTxs, tx1.TxHash) {
					...
					_, err = s.SendTx(t, s.Node(1), signer, 0, s.BaseFeeX2(), nil)
	...
	s.ModifyConsensusTimeout(t, "20s")
