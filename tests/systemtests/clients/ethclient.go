...
// CheckQueued checks if a transaction is queued.
func (ec *EthClient) CheckQueued(
	nodeID string,
	txHash string,
	timeout time.Duration,
) error {
	...
	for {
		...
		case <-ctx.Done():
			return fmt.Errorf("timeout waiting for transaction %s", txHash)
		case <-ticker.C:
			_, queuedTxs, err := ec.TxPoolContent(nodeID)
			if err != nil {
				return fmt.Errorf("failed to get txpool content")
			}

			queuedTxHashes := extractTxHashesSorted(queuedTxs)

			if ok := slices.Contains(queuedTxHashes, txHash); ok {
				return nil
			}
...
// CheckPendingOrCommited checks if a transaction is either pending in the mempool or already committed.
func (ec *EthClient) CheckNotReceived(
	nodeID string,
	txHash string,
	timeout time.Duration,
) error {
	...
	for {
		...
		case <-ctx.Done():
			return nil
		case <-ticker.C:
			pendingTxs, queuedTxs, err := ec.TxPoolContent(nodeID)
			if err != nil {
				return fmt.Errorf("failed to get txpool content")
			}

			// Check in pending txs
			pendingTxHashes := extractTxHashesSorted(pendingTxs)
			if ok := slices.Contains(pendingTxHashes, txHash); ok {
				return fmt.Errorf("tx %s is found in pending txs", txHash)
			}

			// Check in queued txs
			queuedTxHashes := extractTxHashesSorted(queuedTxs)
			if ok := slices.Contains(queuedTxHashes, txHash); ok {
				return fmt.Errorf("tx %s is found in queued txs", txHash)
			}

			// Check if the transaction is already committed
			if _, err = ethCli.TransactionReceipt(context.Background(), common.HexToHash(txHash)); err == nil {
				return fmt.Errorf("tx %s is already committed", txHash)
			}
		}
	}
}
