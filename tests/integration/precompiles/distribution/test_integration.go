...
var (
	...
	minExpRewardOrCommission = sdk.NewDecCoins(sdk.NewDecCoin(testconfig.ExampleAttoDenom, testRewardsAmt))
)
	...
	_ = Describe("Calling distribution precompile from EOA", func() {
		...
		Describe("Execute FundCommunityPool transaction", func() {
			...
			It("should fund multiple coins to the community pool successfully from EOA", func() {
				...
				sendAmt := []cmn.Coin{
					{Denom: s.bondDenom, Amount: fundAmt.BigInt()},
					{Denom: testconfig.OtherCoinDenoms[0], Amount: fundAmt.BigInt()},
					{Denom: testconfig.OtherCoinDenoms[1], Amount: fundAmt.BigInt()},
				}
