	...
	testCases := []struct {
		...
		},
		{
			...
			func() sdk.Tx {
				...
				gas := uint64(200000)
				amount := sdk.NewCoins(sdk.NewCoin(testconfig.ExampleAttoDenom, sdkmath.NewInt(100*int64(gas)))) //#nosec G115
		...
		},
		{
			...
			func() sdk.Tx {
				from := s.GetKeyring().GetAccAddr(0)
				coinAmount := sdk.NewCoin(testconfig.ExampleAttoDenom, sdkmath.NewInt(20))
				amount := sdk.NewCoins(coinAmount)
				gas := uint64(200000)
		...
		},
		{
			...
			func() sdk.Tx {
				...
				txBuilder.SetFeeAmount(sdk.NewCoins(sdk.NewCoin(testconfig.ExampleAttoDenom, sdkmath.NewInt(30))))
				return txBuilder.GetTx()
			}, false, false, false,
		},
		{
