...
describe('Gov Precompile', function () {
    ...
    // Use epix prefix for EpixChain instead of cosmos
    const EPIX_ADDR = 'epix1cml96vmptgw99syqrrz8az79xer2pcgpsl7kpq'
    const GOV_MODULE_ADDR = 'epix10d07y265gmmuvt4z0w9aw880jnsr700j0fas3g'
    ...
    before(async () => {
        ...
        const jsonProposal = buildProposal(EPIX_ADDR)
    ...
    it('deposits on the global proposal', async function () {
        ...
        const depRcpt = await waitWithTimeout(depTx, DEFAULT_TIMEOUT, RETRY_DELAY_FUNC)
    ...
    it('votes on the global proposal', async function () {
        ...
        const voteRcpt = await waitWithTimeout(voteTx, DEFAULT_TIMEOUT, RETRY_DELAY_FUNC)
    ...
    it('cancels a proposal', async function () {
        ...
        const cancelRcpt = await waitWithTimeout(cancelTx, DEFAULT_TIMEOUT, RETRY_DELAY_FUNC)
