...
func CreateEthTx(
	...
	chainID := evmApp.GetEVMKeeper().EvmChainID()
	...
	if privKey != nil {
		evmChainID := evmApp.GetEVMKeeper().ChainConfig().GetChainId()
		signer := ethtypes.LatestSignerForChainID(big.NewInt(int64(evmChainID))) //nolint:gosec // won't exceed int64
