...
The v0.6 form's `baseapp.NewDefaultProposalHandler(mempool, app)` + `SetSignerExtractionAdapter(evmmempool.NewEthSignerExtractionAdapter(...))` pattern is gone. `NewEthSignerExtractionAdapter` was removed; the new `NoCheckProposalTxVerifier` covers the same role (signing checks were already done by `CheckTx` at insert time).
...
> ⚠️ **Set `mempool.type = "app"` in `config.toml`.** CometBFT v0.39 requires the app-side mempool type whenever the application supplies an EVM mempool. The default is `"flood"`; Krakatoa errors out at startup if it's left as the default. This is a one-line patch to each validator's `~/.simapp/config/config.toml` (or your fork's equivalent), applied at the same time as the binary swap. Forks that opt out of Krakatoa (the previous paragraph) don't need this.
