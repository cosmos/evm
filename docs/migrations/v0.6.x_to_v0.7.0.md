...
**`NewNoCheckProposalTxVerifier` is required by Krakatoa.** It's the proposal-tx verifier the proposer hands to `baseapp.NewDefaultProposalHandler` — Krakatoa already validated the txs via `CheckTx` at insert time, so the proposer-side verifier needs to short-circuit the redundant check. If your fork inherited it from the reference `evmd/mempool.go`, just keep it. If you don't have one, copy the reference's implementation (it's a small wrapper that returns `nil` from the `Verify` method) — without it, the proposal handler would re-run `RunMsgs` on every tx and double-cost block proposals.
