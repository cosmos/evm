...
> **Non-18-decimal chains are not fully supported going forward.** `contrib/x/precisebank` has been removed, so non-18-decimal chains should stay on v0.6.x until they can migrate to 18 decimals. Virtual fee collection (5b) remains 18-decimal-only.
