...
**Optimistic execution (optional, independent of Krakatoa and BlockSTM)**
...
## Step 5: Enable Krakatoa, BlockSTM, and/or optimistic execution
...
- **Optimistic execution (5c)** is an independent SDK feature that overlaps tx finalization with the next block's preparation. Independent of 5a and 5b.

The reference `evmd/app.go` at HEAD wires all three.
...
Independent of BlockSTM. SDK feature that lets the app start processing the next block speculatively while the current block is still finalizing. State-breaking on its own — adopting or dropping it requires a coordinated upgrade.
