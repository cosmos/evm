...
**Krakatoa application-layer mempool (optional)**

- [Step 5a: Krakatoa application-layer mempool](#5a-optional-krakatoa-application-layer-mempool)
...
### 5a. (Optional) Krakatoa application-layer mempool

Krakatoa replaces v0.6's `ExperimentalEVMMempool` as the only app-side EVM mempool option. It is **not** required for v0.7 to function — chains that don't wire it fall back to CometBFT's stock mempool, losing app-level `CheckTx` / `RecheckTx` control but otherwise running normally. Independent of BlockSTM.

If your fork wired `ExperimentalEVMMempool` in v0.6, replace the construction with `evmmempool.NewMempool` and the new handler set. The reference is in `evmd/mempool.go`'s `configureEVMMempool`:
