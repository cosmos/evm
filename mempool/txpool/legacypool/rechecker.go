...
type (
	...
	// should be created.
	rechecker struct {
		// pendingCtx is the context that pending pool rechecks should be run
		// on. updated only by pending pool txs running the recheckFn.
		pendingCtx sdk.Context

		// queueCtx is the context that the queue pool rechecks should be run
		// on. updated by pending pool txs running the recheckFn and by queued
		// txs running the recheckFn.
		queueCtx sdk.Context
...
func (r *rechecker) NewQueuedChecker() func(tx *ethtypes.Transaction) error {
	...
	// returned function act on the same context. These invocations of the
	// recheckFn will only persist context updates back to the main queueCtx if
