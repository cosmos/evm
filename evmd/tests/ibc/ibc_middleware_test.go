	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			if tc.expError == "" {
				...
				balAfterCallback := evmApp.Erc20Keeper.BalanceOf(evmCtx, erc20Contract, contractAddr)
...
func (suite *MiddlewareTestSuite) TestOnRecvPacketNativeErc20() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			balAfterTransfer := evmApp.Erc20Keeper.BalanceOf(evmCtx, nativeErc20.ContractAddr, senderEthAddr)
			suite.Require().Equal(
			...
			if tc.withCallback {
				...
				balAfterUnescrow := evmApp.Erc20Keeper.BalanceOf(evmCtx, nativeErc20.ContractAddr, contractAddr)
...
func (suite *MiddlewareTestSuite) TestOnAcknowledgementPacketNativeErc20() {
	...
	for _, tc := range testCases {
		suite.Run(tc.name, func() {
			...
			checkEscrow := func() {
				erc20BalAfterIbcTransfer := evmApp.Erc20Keeper.BalanceOf(evmCtx, nativeErc20.ContractAddr, senderEthAddr)
				suite.Require().Equal(
					new(big.Int).Sub(nativeErc20.InitialBal, sendAmt.BigInt()).String(),
