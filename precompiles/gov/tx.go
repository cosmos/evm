...
func (p *Precompile) SubmitProposal(
	...
	if err != nil {
		return nil, cosmosErrorRegistry.ResolveMsgServerError(p.ABI, SubmitProposalMethod, err, nil).Err
	}
...
func (p *Precompile) CancelProposal(
	...
	if _, err = p.govMsgServer.CancelProposal(ctx, msg); err != nil {
		return nil, cosmosErrorRegistry.ResolveMsgServerError(p.ABI, CancelProposalMethod, err, nil).Err
	}
...
func (p Precompile) Vote(
	...
	if _, err = p.govMsgServer.Vote(ctx, msg); err != nil {
		return nil, cosmosErrorRegistry.ResolveMsgServerError(p.ABI, VoteMethod, err, nil).Err
	}
...
func (p *Precompile) VoteWeighted(
	...
	if _, err = p.govMsgServer.VoteWeighted(ctx, msg); err != nil {
		return nil, cosmosErrorRegistry.ResolveMsgServerError(p.ABI, VoteWeightedMethod, err, nil).Err
	}
